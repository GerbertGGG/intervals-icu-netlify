...
export default {
  async fetch(req, env, ctx) {
    ...
    if (isWatchfacePath(url.pathname)) {
      return handleWatchfaceRequest(req, url, env);
    }

    if (url.pathname === "/sync") {
      return handleSyncRequest(url, env, ctx);
    }
    ...
    if (isWeeklyMailTestPath(url.pathname)) {
      return withWorkerErrorBoundary(() => handleWeeklyMailTestRequest(url, env));
    }
...
function isWatchfacePath(pathname) {
  return pathname === "/watchface" || pathname === "/watchface/";
}

function isWeeklyMailTestPath(pathname) {
...
async function handleWatchfaceRequest(req, url, env) {
...
async function handleSyncRequest(url, env, ctx) {
...
async function handleWeeklyMailTestRequest(url, env) {
