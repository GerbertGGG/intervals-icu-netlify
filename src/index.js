  ...
  BASE: {
    ...
    sessions: [
      ...
      },
      {
        name: "Einheit C",
        exercises: [
          "2×8 Reverse Lunge je Seite",
          "2×10 Single Leg Calf Raise je Seite",
          "2×10 Dead Bug je Seite",
          "2×30s Copenhagen Plank kurz je Seite",
          "2×8–10 Tibialis Raises",
        ],
        progression: [
          [
            "2×6 Reverse Lunge je Seite",
            "2×8 Single Leg Calf Raise je Seite",
            "2×8 Dead Bug je Seite",
            "2×20s Copenhagen Plank kurz je Seite",
            "2×8 Tibialis Raises",
          ],
          [
            "2×8 Reverse Lunge je Seite",
            "2×10 Single Leg Calf Raise je Seite",
            "2×10 Dead Bug je Seite",
            "2×25s Copenhagen Plank kurz je Seite",
            "2×10 Tibialis Raises",
          ],
          [
            "3×8 Reverse Lunge je Seite",
            "3×10 Single Leg Calf Raise je Seite",
            "3×10 Dead Bug je Seite",
            "2×30s Copenhagen Plank kurz je Seite",
            "2×12 Tibialis Raises",
          ],
          [
            "2×6 Reverse Lunge je Seite",
            "2×8 Single Leg Calf Raise je Seite",
  ...
  BUILD: {
    ...
    sessions: [
      ...
      },
      {
        name: "Einheit C",
        exercises: [
          "3×6 Box Step-Down je Seite",
          "2×8 Romanian Deadlift (beidbeinig, moderat)",
          "2×8 Split Squat Isometric Hold (20–30s) je Seite",
          "2×8 Pallof Press je Seite",
          "2×30s Soleus-Wand-Sitz je Seite",
        ],
        progression: [
          [
            "2×6 Box Step-Down je Seite",
            "2×6 Romanian Deadlift (beidbeinig, moderat)",
            "2×20s Split Squat Isometric Hold je Seite",
            "2×8 Pallof Press je Seite",
            "2×20s Soleus-Wand-Sitz je Seite",
          ],
          [
            "3×6 Box Step-Down je Seite",
            "2×8 Romanian Deadlift (beidbeinig, moderat)",
            "2×25s Split Squat Isometric Hold je Seite",
            "2×10 Pallof Press je Seite",
            "2×25s Soleus-Wand-Sitz je Seite",
          ],
          [
            "3×8 Box Step-Down je Seite",
            "3×8 Romanian Deadlift (beidbeinig, moderat)",
            "2×30s Split Squat Isometric Hold je Seite",
            "3×10 Pallof Press je Seite",
            "2×30s Soleus-Wand-Sitz je Seite",
          ],
          [
            "2×6 Box Step-Down je Seite",
...
const BIKE_ALLOWANCE_FACTOR_MAX = 1.0;
...
function buildStrengthMailHtml(sessions, blockState) {
  try {
    ...
    const sessionHtml = sessionList
      .map((session, idx) => renderSession(`Einheit ${String.fromCharCode(65 + idx)}`, session))
      .join("");
    return `<!doctype html>
<html lang="de">
  <head>
...
async function sendWeeklyStrengthMail(env, blockState, strengthCountThisWeek, options = {}) {
  try {
    ...
    const strengthPlan = getStrengthPhasePlan(blockState?.block);
    const availableSessions = Array.isArray(strengthPlan?.sessions) ? strengthPlan.sessions.length : 0;
    const plannedSessions = Math.max(1, Number(strengthPlan?.sessionsPerWeek) || 2);
    const sessionCount = Math.max(1, Math.min(plannedSessions, availableSessions || plannedSessions));
    const sessions = Array.from({ length: sessionCount }, (_, offset) =>
      getStrengthSessionForDay(blockState, normalizedStrengthCount + offset)
    );
...
function buildWeekPreview(
  ...
  try {
    ...
    const deriveWeeklyTrainingPolicy = ({
      overlayMode,
      date,
      keyCompleted,
      longrunCompleted,
      ...
      const safetyReasons = [];
      const reasons = [];
      const forceRest = overlayMode === "LIFE_EVENT_STOP" || overlayMode === "POST_RACE_RAMP_REST";
      if (forceRest) safetyReasons.push("overlay_force_rest");
      const allowLongrun = !forceRest && !LONGRUN_OVERLAY_BLOCKERS.has(overlayMode);
      const allowKey = !forceRest && overlayMode !== "LIFE_EVENT_STOP" && overlayMode !== "POST_RACE_RAMP_REST";
      const allowStrength = !forceRest;
      if (!allowLongrun) reasons.push("longrun_overlay_limited");
      if (!allowKey) reasons.push("key_overlay_limited");
      const longrunCompletedCount = longrunCompleted ? 1 : 0;
      return {
        overlayMode,
        phase: blockState?.block || "BASE",
        eventDistance,
        daysToEvent: eventDateIso ? diffDays(date, eventDateIso) : null,
        week: {
          currentDate: date,
          endDate: addDaysIso(todayIso, 6),
          daysRemaining: Math.max(0, diffDays(date, addDaysIso(todayIso, 6))),
          weekendAhead: [6, 7].some((wd) => listIsoDaysInclusive(date, addDaysIso(todayIso, 6)).some((iso) => isoWeekdayBerlin(iso) === wd)),
        },
        safety: {
          allowKey,
          allowLongrun,
        ...
        budgets: {
          ...
          keyCompleted,
          keyRemaining: Math.max(0, keyBudgetByFrequency - keyCompleted),
          longrunTarget: thisWeekActuals.longrundDone ? 0 : 1,
          longrunCompleted: longrunCompletedCount,
          longrunRemaining: Math.max(0, (thisWeekActuals.longrundDone ? 0 : 1) - longrunCompletedCount),
          runFrequencyMin: fallbackRunTargetRange.min,
          runFrequencyMax: fallbackRunTargetRange.max,
          plannedRunsSoFar,
          strengthTargetSessions: strengthState.targetSessions,
          strengthRemainingSessions: Math.max(0, strengthState.targetSessions - plannedStrengthTotal),
        },
        priorities: {
          preferLongrun: !longrunCompleted && allowLongrun,
          preferKey: keyCompleted < keyBudgetByFrequency && allowKey,
          preferStrength: plannedStrengthTotal < strengthState.targetSessions && allowStrength,
          preferGA: true,
        },
        reasons,
      };
    ...
    const resolveKeyPlan = ({ date, overlayMode, lastKnownKeyDate, lastKnownLongrunDate, keyCountPlanned, policy }) => {
      ...
      const keySpacingGapOk = !lastKnownKeyDate || diffDays(lastKnownKeyDate, date) >= keyMinGapDays;
      const keyLongrunGapOk = !lastKnownLongrunDate || diffDays(lastKnownLongrunDate, date) >= keyLongrunGapDays;
      const canAddKeyByCount = keyCountPlanned < policy.budgets.keyTarget;
      ...
      if (!policy?.safety?.allowKey) reasons.push("policy_disallow_key");
      if (!policy?.safety?.allowKey) hardBlockReasons.push("policy_disallow_key");
      if (!keyDateGateOk) reasons.push("next_allowed_not_reached");
      if (!keySpacingGapOk) reasons.push("min_gap_not_reached");
      if (!keyLongrunGapOk) reasons.push("longrun_proximity_block");
      if (!canAddKeyByCount) reasons.push("weekly_budget_exhausted");
      if (!dayIdle) reasons.push("day_already_has_run");
      if (!keySessionReady) reasons.push("session_not_ready");
      if (overlayBlocksPlannedKey) hardBlockReasons.push("post_race_ramp_key_block");
      if (!keySessionReady) hardBlockReasons.push("session_not_ready");
      const done = policy?.budgets?.keyRemaining === 0;
      const allowedOnDate = keyAllowedOnDate && keyLongrunGapOk && canAddKeyByCount && dayIdle && keySessionReady;
      const weekDaysLeft = Math.max(0, diffDays(date, addDaysIso(todayIso, 6)));
      const nextAllowedInRange = keyNextAllowedIso && keyNextAllowedIso <= addDaysIso(todayIso, 6);
      const keyOpenThisWeekSoon = !done && !allowedOnDate && (nextAllowedInRange || weekDaysLeft >= 2);
      const hardBlocked = !done && hardBlockReasons.length > 0;
      const status = done
        ? "done"
        : allowedOnDate
          ? "due"
      ...
      return {
        ...
        hardBlocked,
        allowedOnDate,
        nextAllowedIso: keyNextAllowedIso,
        minGapDays: keyMinGapDays,
        keyType: resolvedKeyType || null,
        session: keyCompliance?.explicitSession || null,
        weeklyBudgetRemaining: Math.max(0, policy?.budgets?.keyRemaining ?? 0),
        weekUrgencyScore: done ? 0 : (allowedOnDate ? 90 : (status === "not_today" ? 70 : 40)),
        reasons,
      };
    };
    const resolveLongrunPlan = ({ date, overlayMode, policy, nearestKeyDistance, keyPlan }) => {
      const reasons = [];
      const doneThisWeek = policy?.budgets?.longrunRemaining === 0 || longrunPlanned;
      const overlayBlocked = !policy?.safety?.allowLongrun || LONGRUN_OVERLAY_BLOCKERS.has(overlayMode);
      const keyProximityBlocked = nearestKeyDistance < keyLongrunGapDays;
      const longrunGapOk = !lastLongrunDateBeforeToday || diffDays(lastLongrunDateBeforeToday, date) >= 4;
      const blocked = overlayBlocked || longrunBlocked || keyProximityBlocked || !longrunGapOk;
      if (overlayBlocked) reasons.push("overlay_blocked");
      if (longrunBlockedByFatigue) reasons.push("fatigue_blocked");
      if (longrunBlockedBySpikeGuard) reasons.push("spike_guard_blocked");
      if (keyProximityBlocked) reasons.push("key_proximity_blocked");
      if (!longrunGapOk) reasons.push("min_gap_not_reached");
      if (doneThisWeek) reasons.push("weekly_budget_exhausted");
      const daysLeftInWeek = Math.max(0, diffDays(date, addDaysIso(todayIso, 6)));
      const isWeekendWindow = [6, 7].includes(isoWeekdayBerlin(date));
      const phase = String(policy?.phase || "BASE").toUpperCase();
      const longrunBacklog = Math.max(0, Number(policy?.budgets?.longrunRemaining ?? 0));
      let urgency = 30;
      if (phase === "BASE") urgency += 8;
      if (phase === "BUILD") urgency += 12;
      if (phase === "PEAK") urgency += 16;
      if (isWeekendWindow) urgency += 15;
      if (daysLeftInWeek <= 2 && longrunBacklog > 0) urgency += 20;
      if (keyPlan?.status === "not_today") urgency += 10;
      if (keyPlan?.status === "due") urgency -= 8;
      if (eventDateIso) {
        const dte = diffDays(date, eventDateIso);
        if (dte <= 10) urgency -= 18;
      }
      const longrunPriority = urgency >= 70 ? "due" : (urgency >= 48 ? "important" : "optional");
      const status = doneThisWeek ? "done" : (blocked ? "blocked" : longrunPriority);
      return {
        status,
        doneThisWeek,
        targetMin: longRunTargetMin,
        progressionTargetMin: longRunProgressionTargetMin,
        safetyCapMin: longRunSafetyCapMin,
        blocked,
        urgencyScore: clampInt(String(Math.round(urgency)), 0, 100),
        isWeekendWindow,
        daysLeftInWeek,
        reasons,
      };
    };
    const resolvePrimaryRunPriority = ({
      policy,
      keyPlan,
      longrunPlan,
      strengthState,
      ...
      const candidates = [];
      if (keyPlan?.status === "due") {
        let score = 72;
        if (policy?.phase === "BUILD" || policy?.phase === "PEAK") score += 8;
        if (eventDateIso && diffDays(dayContext?.date, eventDateIso) <= 12) score += 10;
        if (longrunPlan?.status === "due") score -= 8;
        if (longrunPlan?.status === "important") score -= 5;
        candidates.push({ candidate: "KEY", score, reason: "key_due_priority" });
      } else if (keyPlan?.status === "optional") {
        candidates.push({ candidate: "KEY", score: 40, reason: "key_optional_window" });
      }

      if (["due", "important"].includes(longrunPlan?.status)) {
        let score = longrunPlan.status === "due" ? 75 : 62;
        if (dayContext?.isWeekend) score += 8;
        if (keyPlan?.status === "due") score -= 10;
        if (keyPlan?.status === "not_today") score += 9;
        candidates.push({ candidate: "LONGRUN", score, reason: `longrun_${longrunPlan.status}` });
      }

      const strengthAnchor = strengthState?.priority === "anchor";
      if (strengthAnchor && !policy?.safety?.forceRest) {
        const score = 50 + (dayContext?.isStrengthPreferred ? 6 : 0);
        candidates.push({ candidate: "STRENGTH", score, reason: "strength_anchor_guard" });
      }

      if (!candidates.length) return { candidate: "GA", reason: "ga_default", scored: [] };
      candidates.sort((a, b) => b.score - a.score);
      return { candidate: candidates[0].candidate, reason: candidates[0].reason, scored: candidates };
    };
    const decideSessionForDay = ({
      ...
      const plannedRunsSoFar = days.filter((entry) =>
        entry.status === "PLANNED" && ["GA", "KEY", "LONGRUN"].includes(entry.sessionType)
      ).length;
      const plannedStrengthTotal = thisWeekActuals.strengthCount + plannedStrengthCount;
      const policy = deriveWeeklyTrainingPolicy({
        overlayMode,
        date,
        keyCompleted: keyCountPlanned,
        longrunCompleted: thisWeekActuals.longrundDone || longrunPlanned,
        plannedRunsSoFar,
        plannedStrengthTotal,
      });
      ...
      if (overlayMode === "TAPER") {
        ...
        const nearestKeyDistance = allKnownKeyDates.length
          ...
          : 99;
        const longrunPlan = resolveLongrunPlan({ date, overlayMode, policy, nearestKeyDistance, keyPlan });
        const remainingStrengthSessions = Math.max(0, policy.budgets.strengthRemainingSessions);
        const strengthNeed = remainingStrengthSessions > 0;
        const strengthPriority = (() => {
          if (!strengthNeed || !policy.safety.allowStrength) return "optional";
          if (["BASE", "BUILD"].includes(String(policy?.phase || "").toUpperCase()) && remainingStrengthSessions >= 1) return "anchor";
          if (String(policy?.overlayMode || "").toUpperCase() === "TAPER") return "optional";
          return "supporting";
        })();
        ...
        traceInput.key = keyPlan;
        traceInput.longrun = longrunPlan;
        traceInput.strength.priority = strengthPriority;
        const primaryRunPriority = resolvePrimaryRunPriority({
          policy,
          keyPlan,
          longrunPlan,
          strengthState: traceInput.strength,
          dayContext: {
            date,
            dayIndex,
            isWeekend: [6, 7].includes(weekday),
            isStrengthPreferred: isStrengthPref,
          },
        });
        traceInput.priorityResolution = primaryRunPriority;
        const candidates = [
          {
            candidate: "KEY",
            status: keyPlan.status,
            reason: keyPlan.reasons.join("|") || "eligible",
          },
          {
            candidate: "LONGRUN",
            status: longrunPlan.status,
            reason: longrunPlan.reasons.join("|") || "eligible",
          },
          {
            candidate: "STRENGTH",
            status: strengthNeed && policy.safety.allowStrength ? strengthPriority : "blocked",
            reason: strengthNeed ? (isStrengthPref ? "strength_window_open" : `strength_${strengthPriority}`) : "target_met",
          },
          {
            candidate: "GA",
            status: "optional",
          ...
          {
            candidate: "REST",
            status: policy.safety.forceRest ? "due" : "optional",
            reason: policy.safety.forceRest ? "policy_force_rest" : "optional_rest",
          },
        ];
        decisionCandidates.push(...candidates);
        if (primaryRunPriority.candidate === "KEY" && keyPlan.allowedOnDate) {
          decisionTrace.push(`key_selected:${primaryRunPriority.reason}`);
          sessionType = "KEY";
          intensity = "HIGH";
          keyType = keyPlan.keyType || null;
          sessionLabel = `Key – ${formatKeyType(keyType || keyCompliance?.plannedKeyType || "steady")}`;
          plannedKeyDates.push(date);
          if (!thisWeekActuals.keyDone && thisWeekActuals.keyDates.length === 0 && thisWeekDays.length > 0) note = "Key aus dieser Woche nachholen";
        } else if (primaryRunPriority.candidate === "LONGRUN" && ["due", "important"].includes(longrunPlan.status)) {
          decisionTrace.push(`longrun_selected:${primaryRunPriority.reason}`);
          sessionType = "LONGRUN";
          intensity = "MED";
          sessionLabel = `Longrun ~${longrunPlan.targetMin}′`;
          longrunPlanned = true;
          plannedLongrunDates.push(date);
        } else if (primaryRunPriority.candidate === "STRENGTH" && strengthNeed && policy.safety.allowStrength) {
          decisionTrace.push("strength_selected:policy_priority");
          ...
          policy.budgets.keyRemaining === 0
          && (longrunPlanned || thisWeekActuals.longrundDone)
          && plannedRunsSoFar >= policy.budgets.runFrequencyMin
        ) {
    ...
    const policyAcceptanceScenarios = [
      {
        ...
        coachingConcern: "Key heute nicht erzwingen; Restwochen-Dringlichkeit muss sichtbar bleiben",
      },
      {
        ...
        longrun: "important",
        expectedDay: "LONGRUN",
        reasons: ["longrun_important"],
        expectedPriority: ["LONGRUN"],
        expectedNonPriority: ["KEY"],
        maxAcceptableAlternative: ["STRENGTH"],
      ...
      },
      {
        ...
        maxAcceptableAlternative: ["LONGRUN"],
      },
      {
        id: "BUILD_KEY_NOT_TODAY_LONGRUN_OPEN",
        key: "not_today",
        longrun: "due",
        expectedDay: "LONGRUN",
        reasons: ["next_allowed_not_reached", "longrun_due"],
        expectedPriority: ["LONGRUN"],
        expectedNonPriority: ["KEY"],
        maxAcceptableAlternative: ["GA"],
      },
      {
        id: "BASE_STRENGTH_ANCHOR_GAP",
        key: "optional",
        longrun: "optional",
        expectedDay: "STRENGTH",
        reasons: ["strength_anchor_guard"],
        expectedPriority: ["STRENGTH"],
        maxAcceptableAlternative: ["GA"],
      },
      {
        id: "LOW_RUN_FREQUENCY_MULTI_OPEN",
        key: "not_today",
        longrun: "due",
        expectedDay: "LONGRUN",
        reasons: ["longrun_due"],
        expectedPriority: ["LONGRUN"],
        expectedNonPriority: ["KEY", "STRENGTH"],
        maxAcceptableAlternative: ["GA"],
      },
      {
        id: "TAPER_RECOVERY_SHARPENER",
        key: "optional",
        longrun: "blocked",
        expectedDay: "RECOVERY",
        reasons: ["overlay:taper"],
        expectedPriority: ["RECOVERY"],
        maxAcceptableAlternative: ["KEY"],
        coachingConcern: "im Taper standardmäßig Erholung, kurzer Schärfereiz nur als kontrollierte Alternative",
      },
...
function buildWeeklyFocus(ctx, todayIso, blockState, keyCompliance, runFloorState, weekMemories, manualFocus) {
  try {
    ...
    if (holidayFocus) {
      ...
      reason = "Urlaub aktiv — Trainingsziele pausieren, kein Trainingsdruck.";
    ...
    const mapping = {
      ...
      holiday: [
        "Urlaub aktiv — Trainingsziele pausieren",
        "Heute kein Trainingsfokus, kein Pflicht-Key, kein Pflicht-Longrun",
        "Wiedereinstieg nach dem Urlaub schrittweise planen",
      ],
    };
  ...
  console.log("[debug:eventDistance]", {
    ...
    await timedSync(dayPerf, "renderAndPostProcessingMs", async () => {
      ...
      weeklyFocus = isMondayIso(day)
        ...
        : null;
      if (isHolidayOverlayMode(runFloorState?.overlayMode)) {
        weeklyFocus = null;
      }
...
function deriveLongrunStatusLineFromResolver({ longrunPlan, resolvedDecision }) {
  ...
  if (isHolidayOverlayMode(longrunPlan?.overlayMode)) return "Longrun-Status: Urlaub aktiv — kein Longrun-Fokus.";
...
function deriveKeyStatusLineFromResolver(keyPlan, overlayMode = null) {
  ...
  if (isHolidayOverlayMode(overlayMode || keyPlan?.overlayMode)) {
    return "Key-Status: Urlaub aktiv — kein Key-Fokus.";
  }
...
function deriveStrengthStatusLineFromState(strengthState) {
  ...
  if (isHolidayOverlayMode(strengthState?.overlayMode)) return "Strength-Status: Urlaub aktiv — kein Strength-Fokus.";
...
function deriveTrainingStatusFromPolicy({
  ...
  return [
    ...
    Number.isFinite(policy?.budgets?.plannedRunsSoFar) && Number.isFinite(policy?.budgets?.runFrequencyMin) && Number.isFinite(policy?.budgets?.runFrequencyMax)
      ? holidayMode
        ? `Run-Frequenz: ${policy.budgets.plannedRunsSoFar} geplant (Urlaub aktiv — Trainingsziele pausiert).`
...
function buildNextRunRecommendation({
  ...
  if (overlay === "LIFE_EVENT_STOP") {
    ...
    if (plannedTypeUpper === "KEY") next = "Urlaub aktiv: heute kein Key-Fokus.";
    else if (plannedTypeUpper === "LONGRUN") next = "Urlaub aktiv: heute kein Longrun-Fokus.";
    else if (plannedTypeUpper === "STRENGTH") next = "Urlaub aktiv: heute kein Strength-Fokus.";
    else next = "Urlaub aktiv — Trainingsziele pausieren.";
    ...
    next = "Urlaub ist Urlaub — Wiedereinstieg nach dem Urlaub.";
...
function evaluateNarrativeConsistency({
  ...
  if (["blocked", "not_today"].includes(keyPlan?.status) && combinedText.includes("key-fenster offen")) {
  ...
  if (["due", "important"].includes(longrunPlan?.status) && /longrun-status:\s*(blocked|done)/.test(trainingStatusText)) {
...
function runPolicyAcceptanceScenarios({ scenarios = [], days = [], onTiming = null }) {
  ...
  const scenarioMatchers = {
    ...
    BUILD_KEY_NOT_TODAY_LONGRUN_OPEN: (entry) => entry?.status === "PLANNED" && entry?.sessionType === "LONGRUN",
    BASE_STRENGTH_ANCHOR_GAP: (entry) => entry?.status === "PLANNED" && entry?.sessionType === "STRENGTH",
    LOW_RUN_FREQUENCY_MULTI_OPEN: (entry) => entry?.status === "PLANNED" && entry?.sessionType === "LONGRUN",
    TAPER_RECOVERY_SHARPENER: (entry) => String(entry?.overlayMode || "") === "TAPER",
  ...
  const evaluateScenarioNarrativeBlockConsistency = ({ selected, policy, keyPlan, longrunPlan, strengthState, mainReasons = [] }) => {
    ...
    if (["blocked", "not_today"].includes(keyPlan?.status) && selectedType === "KEY") {
      trainingStatusPass = false;
      mismatches.push("training_status_key_blocked_vs_selected_key");
    }
    if (["due", "important"].includes(longrunPlan?.status) && selectedType === "KEY" && keyPlan?.status !== "due") {
      trainingStatusPass = false;
  ...
  const evaluateScenarioCoachConsistency = ({ selected, keyPlan, longrunPlan }, scenario) => {
    ...
    if (keyPlan?.status === "due" && selectedType === "LONGRUN" && longrunPlan?.status !== "due") {
      coachingConcerns.push("coach_key_due_but_longrun_selected");
    }
    if (["due", "important"].includes(longrunPlan?.status) && selectedType === "STRENGTH") {
...
function deriveBottomLineFromDecision({ narrativeContext, explicitSessionShort }) {
  ...
  if (holidayMode) return "Urlaub aktiv — Trainingsziele pausieren. Wiedereinstieg nach dem Urlaub.";
  if (sessionType === "KEY") {
...
function deriveRecommendationFromDecision({ narrativeContext, distanceDiagnostics, gapRecommendations }) {
  ...
  if (holidayMode) {
    rec.push("Urlaub aktiv — Trainingsziele pausieren.");
    rec.push("Heute kein Trainingsfokus.");
    rec.push("Kein Key-/Longrun-/Strength-Druck.");
    rec.push("Urlaub ist Urlaub.");
    if (distanceDiagnostics?.readiness != null) rec.push(`Status: Readiness ${distanceDiagnostics.readiness}/100 (nur Info, ohne Trainingspflicht).`);
    rec.push("Wiedereinstieg nach dem Urlaub.");
    return rec;
  }
  ...
  } else if (keyPlan?.status === "not_today") {
    rec.push(`Key heute nicht priorisiert${keyPlan?.nextAllowedIso ? ` (frühestens ${keyPlan.nextAllowedIso})` : ""} – Restwoche aktiv im Blick behalten.`);
  }
...
function buildComments(
  ...
  if (holidayReportMode) {
    const compactHolidayStatus = [
      statusLines.find((line) => /^Overlay:/i.test(String(line || ""))) || "Overlay: LIFE_EVENT_HOLIDAY (Urlaub aktiv)",
      statusLines.find((line) => /^RunFloor:/i.test(String(line || ""))) || null,
      "Trainingsziele pausiert — kein Trainingsdruck.",
    ].filter(Boolean);
    addDecisionBlock("STATUS", compactHolidayStatus.slice(0, 3));
  } else {
