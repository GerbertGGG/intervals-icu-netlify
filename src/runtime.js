import { clampInt, json } from './http-helpers.js';
...
const __default_export__ = {
  async fetch(req, env, ctx) {
    ...
    if (isWatchfacePath(url.pathname)) {
      return handleWatchfaceRequest(req, url, env, { buildWatchfacePayload });
    }
    ...
    if (url.pathname === "/sync-step") {
      return withWorkerErrorBoundary(() => handleStepSync(req, env, ctx));
    }

    if (isWeeklyMailTestPath(url.pathname)) {
      return withWorkerErrorBoundary(() => handleWeeklyMailTestRequest(url, env, { sendWeeklyStrengthMail }));
    }
      ...
      },
      {
        ...
        progression: [
          ...
          [
            "2×6 Step-Ups",
            "2×6 Single Leg Deadlift",
            "2×6 Hip Hike je Seite",
            "2×8 Mini-Band Lateral Walk je Seite",
            "2×8 Monster Walk",
            "2×20s Seitstütz",
          ],
        ],
      ...
      },
      {
        ...
        progression: [
          [
            ...
            "2×15s Split Squat Isometric Hold je Seite",
            "2×6 Pallof Press je Seite",
            "2×15s Soleus-Wand-Sitz je Seite",
          ],
          [
            "3×6 Box Step-Down je Seite",
            "3×6 Romanian Deadlift (beidbeinig, moderat)",
            "2×20s Split Squat Isometric Hold je Seite",
            "2×8 Pallof Press je Seite",
            "2×20s Soleus-Wand-Sitz je Seite",
          ],
          ...
          [
            "2×6 Box Step-Down je Seite",
            "2×6 Romanian Deadlift (beidbeinig)",
            "2×15s Split Squat Isometric Hold je Seite",
            "2×6 Pallof Press je Seite",
            "2×15s Soleus-Wand-Sitz je Seite",
          ],
...
function buildStrengthMailHtml(sessions, blockState) {
  try {
    ...
    const cycleWeek = Number(safeSessions[0]?.cycleWeek) + 1 || 1;
    const renderSession = (title, session) => {
      ...
      return `
        <div style="margin:0 0 18px 0;padding:14px;border:1px solid #374151;border-radius:10px;background:#1f2937;">
          <h3 style="margin:0 0 8px 0;font-size:26px;color:#f3f4f6;">${escapeHtml(title)} — ${escapeHtml(session.name || "Einheit")}${deloadLabel}</h3>
          <p style="margin:0 0 10px 0;color:#d1d5db;">Dauer: ca. ${(session.durationMin || [15, 20])[0]}–${(session.durationMin || [15, 20])[1]} Min</p>
          <ul style="margin:0 0 12px 18px;padding:0;">${exerciseItems}</ul>
          <p style="margin:0 0 8px 0;font-weight:600;color:#f3f4f6;">Mobility danach:</p>
...
function detectStrength(a) {
  ...
  return { matched, typeMatch, tagMatch, keywordMatch, reason };
}
...
function countStrengthSessionsWeekToYesterday(ctx, todayIso) {
  const todayWeekday = isoWeekdayBerlin(todayIso);
  const weekStart = addDaysIso(todayIso, -(todayWeekday - 1));
  const yesterdayIso = addDaysIso(todayIso, -1);
  if (!isIsoDate(weekStart) || !isIsoDate(yesterdayIso) || yesterdayIso < weekStart) return 0;
  let count = 0;
  for (const iso of listIsoDaysInclusive(weekStart, yesterdayIso)) {
...
// ─── runtime-sync-engine ───
  ...
  console.log("[debug:eventDistance]", {
    ...
    if (day === oldest) {
      strengthCountThisWeek = countStrengthSessionsWeekToYesterday(ctx, day);
    }
    ...
    const isRaceDayToday = Boolean(
      isIsoDate(String(blockState?.eventDateISO || "")) && String(blockState?.eventDateISO) === day
    );
...
// ─── runtime-commentary-and-analysis ───
...
// ─── runtime-integrations-and-hooks ───
  }
...
export { __default_export__ as default, __test, __internalTestHooksForRepoTestsOnly };
